                    ...
                    <div class="col-md-12">
                        ...
                        try {
                            ...
                            foreach($result as $row) {
                                ...
                                $thecompname = "Empresa no disponible";
                                $complogo = null;
                                ...
                                try {
                                    $date_obj = date_create_from_format('d/m/Y', $closingdate);
                                    $post_date = $date_obj ? date_format($date_obj, 'd') : '--';
                                    $post_month = $date_obj ? date_format($date_obj, 'F') : 'N/A';
                                    $post_year = $date_obj ? date_format($date_obj, 'Y') : '----';
                                } catch (Exception $e) {
                                    $post_date = "--"; $post_month = "N/A"; $post_year = "----";
                                }

                                $stmtb = $conn->prepare("SELECT * FROM tbl_users WHERE member_no = :compid AND role = 'employer'");
                                $stmtb->bindParam(':compid', $company_id);
                                $stmtb->execute();
                                ...
                                if ($type == "Freelance") { $sta = '<div class="job-label label label-success">Freelance</div>'; } 
                                elseif ($type == "Part-time") { $sta = '<div class="job-label label label-danger">Part-time</div>'; } 
                                elseif ($type == "Full-time") { $sta = '<div class="job-label label label-warning">Full-time</div>'; } 
                                else { $sta = ''; }
                        ?>
